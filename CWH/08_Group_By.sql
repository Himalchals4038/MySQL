...
select gender, min(marks) as min_marks, max(marks) as max_marks, avg(marks) as avg_marks, sum(marks) as total_marks from student group by gender;
select name, length(name) as length_name, lower(name) as lower_name, upper(name) as upper_name from student;
select name, concat("Student ", lower(name)) as new_name from student;
select * from student;
drop table student;