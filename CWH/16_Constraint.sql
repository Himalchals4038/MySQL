...
create table hostel(
    ...
    constraint fk_user foreign key(id) references student(id)
);
INSERT INTO hostel (id, name, email, password, room_no) VALUES
(1, 'Rohan Mehra', 'rohan.mehra@example.com', 'pass123', 101),
(3, 'Amit Singh', 'amit.singh@example.com', 'amit@123', 102),
(5, 'Vikram Kumar', 'vikram.kumar@example.com', 'vikram!pass', 201),
(2, 'Priya Sharma', 'priya.sharma@example.com', 'securepass', 202),
(4, 'Sneha Patel', 'sneha.patel@example.com', 'sneha_pass', 301);
select * from hostel;
update student set email = 'rohan@college.com' where id = 1;
select name, email, grade, marks from student where id in (select id from hostel where room_no=101);
drop table student;
drop table hostel;
