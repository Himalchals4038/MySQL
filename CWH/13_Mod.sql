...
SELECT id, name, CASE WHEN MOD(id, 2) = 0 THEN 'Even' ELSE 'Odd' END AS id_type FROM resident;
SELECT CASE WHEN MOD(id, 2) = 0 THEN 'Even' ELSE 'Odd' END AS id_type, COUNT(id) AS total_residents, GROUP_CONCAT(name) as residents FROM resident GROUP BY id_type;
drop table resident;