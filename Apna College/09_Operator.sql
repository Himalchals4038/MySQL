...
-- select address, count(*) from resident group by gender having age>20;
select address, count(*) from resident group by address having max(age)<22;
select address, avg(age) as avg_age from resident group by address having avg_age>20;
update professor set rating=4.1 where id in (select id from resident where address='Pune 1' or address='Pune 2');
update professor set rating=3.8 where id in (select id from resident where age>20 and gender='Female');
drop table resident;
